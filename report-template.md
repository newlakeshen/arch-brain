...
| 维度 | 评分(1-10) | 判定 | 关键发现 | 核心证据（文件 + 指标） |
|------|-----------|------|---------|----------------------|
| [维度名] | [分数] | ✅/⚠️/🚨 | [一句话发现] | [关键文件:行号 + 量化指标] |
...
| # | 位置 | 发现（含量化指标） | 影响 |
|---|------|------------------|------|
| 1 | [file:lines] | [具体发现 + 数字] | [具体影响，用人日/风险/成本量化] |
| 2 | [file:lines] | [具体发现 + 数字] | [具体影响] |

**现状描述:** [基于上述证据的客观描述]

**评分依据:** [引用评分标准 + 映射到具体证据，说明为何是此分数而非更高/更低]

**瓶颈分析:**（7+ 分维度可省略此节，改为简述优势和剩余优化空间）
- **根因:** [问题的根本原因，而非表象]
- **量化影响:** [用数字说明严重程度，如 LOC、重复次数、修改成本]
- **对比基准:** [当前实践 vs 行业最佳实践，差距在哪]

**改进建议:**
- [具体可操作的建议 + 预估改善效果]

> 如维度含子维度（如可扩展性的扩展轴），使用以下扩展表格：
>
> | 子维度 | 评分 | 证据摘要（2-3 句含指标） | 关键文件 | 量化影响 |
> |--------|------|------------------------|---------|---------|
> | [name] | [N/10] | [具体描述] | [file:lines] | [人日/风险] |
